...
def pr_clone(
    ...
    The origin holds ``main``, ``feature`` one commit above it, a stale
    ``old`` below ``main``, and ``refs/pull/<n>/head`` one commit above
    ``feature``. The clone lands at ``<session_root>/<name>``. Returns
    the head sha. With ``on_main`` the pull request forks from ``main``
    instead, ``main`` then moves on, and ``feature`` merges into it.
    """
