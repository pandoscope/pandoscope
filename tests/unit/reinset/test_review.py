...
def test_task_is_the_pass_files_prompt_block_with_the_placeholders_filled(
    ...
    task = review_task(session_root, Review("spec-fidelity", "opus", 261))
    assert task == (
...
def test_unknown_pull_request_leaves_the_number_placeholder(
    session_root: Path,
) -> None:
    write_pass(session_root)
    task = review_task(session_root, Review("spec-fidelity", "opus", None))
    assert "pull request <n> as tier opus" in task
    assert "-pr<n>" in task
...
def test_missing_pass_file_is_a_review_error(session_root: Path) -> None:
    with pytest.raises(ReviewError, match=r"spec-fidelity\.md"):
        review_task(session_root, Review("spec-fidelity", "opus", 1))
